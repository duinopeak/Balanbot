...
void setValues(char *input) {
  if (input[0] == 'A' && input[1] == ';') { // Abort
    ...
    Serial.println(F("Now turn the Balanbot and the satellite receiver off by removing the power. The next time it turns on it will start the binding process with the satellite receiver"));
